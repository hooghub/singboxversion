#!/bin/bash
# Sing-box 一键部署脚本（修复版 / IPv6-only 友好）
# - 支持：域名 + Let's Encrypt 证书 / 自签固定域名 www.epple.com
# - 支持：VLESS-TLS + Hysteria2 + VLESS-REALITY
# - 修复：
#   1) IPv6-only 下 curl -4 导致 set -e 退出
#   2) sing-box.app/deb-install 在 IPv6-only 下失败 -> 改为 GitHub Release 安装（强制 IPv6）
#   3) 自签证书 SAN 不再硬塞空 IPv4
#   4) 模式2 节点使用 IPv6，URI 自动加 []，避免导入失败
# Author: Chis (优化 by ChatGPT, 修复 by ChatGPT)
...
# --------- 自动安装依赖 ---------
REQUIRED_CMDS=(curl ss openssl qrencode dig systemctl bash socat cron ufw tar)
MISSING_CMDS=()
...
if [[ ${#MISSING_CMDS[@]} -gt 0 ]]; then
  ...
  for cmd in "${MISSING_CMDS[@]}"; do
    case "$cmd" in
      ...
      qrencode|socat|ufw|tar) INSTALL_PACKAGES+=("$cmd") ;;
...
# --------- 安装 sing-box（改为 GitHub Release，强制 IPv6 优先，IPv4 兜底） ---------
install_singbox() {
  if command -v sing-box >/dev/null 2>&1; then
    return 0
  fi

  echo ">>> 安装 sing-box（GitHub Release）..."
  ...
  # 取最新版本（优先 IPv6，失败则尝试 IPv4）
  local VER
  VER="$(curl -6 -fsSL https://api.github.com/repos/SagerNet/sing-box/releases/latest 2>/dev/null \
        | grep -m1 '"tag_name"' | cut -d '"' -f4 || true)"
  if [[ -z "$VER" ]]; then
    VER="$(curl -4 -fsSL https://api.github.com/repos/SagerNet/sing-box/releases/latest 2>/dev/null \
          | grep -m1 '"tag_name"' | cut -d '"' -f4 || true)"
  fi

  if [[ -z "$VER" ]]; then
    echo "[✖] 获取 sing-box 最新版本失败（无法访问 GitHub API）"
    exit 1
  fi

  echo ">>> 最新版本: $VER"

  local URL="https://github.com/SagerNet/sing-box/releases/download/${VER}/sing-box-${VER#v}-linux-${ARCH}.tar.gz"
...
if command -v ufw &>/dev/null; then
  ...
  ufw allow "$VLESS_R_PORT"/tcp || true
  ufw allow "$VLESS_R6_PORT"/tcp || true
...
command -v qrencode &>/dev/null && echo "$VLESS_REALITY_URI" | qrencode -t ansiutf8 || true
